init python:
    ...
    for track in music_room.tracks:
        if music_room.variable_mode:
            mr.add(track["file"], always_unlocked=False)
        else:
            mr.add(track["file"], music_room.unlocked)
    ...
    use game_menu(_("Music Room")):
        ...
        fixed:
            ...
            vbox:
                ...
                text "Currently Playing: {}".format(is_playing if is_playing else "Nothing"):
                    xalign 0.5
            ...
            frame:
                ...
                grid mr_rows mr_cols:
                    ...
                    if next_mr_page > (len(music_room.tracks) - 1) // mr_cells:
                        $ next_mr_page = 0
                    ...
                    for track in music_room.tracks:
                        ...
                        if i <= (mr_page+1)*mr_cells and i>mr_page*mr_cells:
                            if music_room.variable_mode:
                                $ track_unlocked = getattr(persistent, track["variable"], False)
                            else:
                                $ track_unlocked = mr.is_unlocked(track["file"])
                ...
                if (len(music_room.tracks)>mr_cells):
                    $ pages = (len(music_room.tracks) + mr_cells - 1) // mr_cells
